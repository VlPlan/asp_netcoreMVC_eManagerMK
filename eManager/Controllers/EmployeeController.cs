...
namespace eManager.Controllers
{
    ...
    public class EmployeeController : Controller
    {
        ...
        public EmployeeController(IEmployeeService employeeService, IToastNotification toastNotification, CloudBlobClient cloudBlobClient, IUserService userService, IHostingEnvironment hostingEnvironment, IClientService clientService, IActivityService activityService)
        {
        ...
        public IActionResult UpdateEmployeeInfo(EmployeeViewModel employeeToView, IFormFile Medical, IFormFile IdCard, IFormFile M1M2, IFormFile HSafety)
        {
            ...
            if (Medical != null)
            {
                string FileName = Medical.FileName;
                //var container = _cloudBlobClient.GetContainerReference("ognootporno");
                //var blob = container.GetBlockBlobReference(FileName);

                //blob.UploadFromStream(Medical.OpenReadStream());
                ...
                var filename = ContentDispositionHeaderValue.Parse(Medical.ContentDisposition).FileName.Trim('"');
                var path = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "UploadedFiles",Medical.FileName);
                using (System.IO.Stream stream = new FileStream(path, FileMode.Create))
                {
                    Medical.CopyTo(stream);
                }
            ...
            if (M1M2 != null)
            {
                string FileName = M1M2.FileName;
                //var container = _cloudBlobClient.GetContainerReference("ognootporno");
                //var blob = container.GetBlockBlobReference(FileName);

                //blob.UploadFromStream(M1M2.OpenReadStream());
                ...
                var filename = ContentDispositionHeaderValue.Parse(M1M2.ContentDisposition).FileName.Trim('"');
                var path = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "UploadedFiles", M1M2.FileName);
                using (System.IO.Stream stream = new FileStream(path, FileMode.Create))
                {
                    M1M2.CopyTo(stream);
                }
            ...
            if (HSafety != null)
            {
                string FileName = HSafety.FileName;
                //var container = _cloudBlobClient.GetContainerReference("ognootporno");
                //var blob = container.GetBlockBlobReference(FileName);

                //blob.UploadFromStream(HSafety.OpenReadStream());
                ...
                //}
                var filename = ContentDispositionHeaderValue.Parse(HSafety.ContentDisposition).FileName.Trim('"');
                var path = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "UploadedFiles", HSafety.FileName);
                using (System.IO.Stream stream = new FileStream(path, FileMode.Create))
                {
                    HSafety.CopyTo(stream);
                }
            ...
            if (IdCard != null)
            {
                ...
                //blob.UploadFromStream(IdCard.OpenReadStream());
                ...
                var filename = ContentDispositionHeaderValue.Parse(IdCard.ContentDisposition).FileName.Trim('"');
                var path = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "UploadedFiles", IdCard.FileName);
                using (System.IO.Stream stream = new FileStream(path, FileMode.Create))
                {
                    IdCard.CopyTo(stream);
                }
        ...
        public IActionResult AddActivity(ActivityViewModel activityModel)
        {
           ...
           activityModel.ClientName = activityClient.Name;           
            ...
            activityModel.ActivityStatusViewModel = ActivityStatusViewModel.Моментално;
        ...
        public IActionResult ConfirmActivity(int activityId)
        {
           ...
           _activityService.CompleteActivity(activityId, user.Id, ActivityStatusViewModel.Завршено);
