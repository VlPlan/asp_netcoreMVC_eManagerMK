    ...
    public class ProcurementService : IProcurementService
    {
        ...
        public int UploadProcurement(int procurementId, int productId, string userId, out string productName)
        {
            try
            {
                ...
                Procurement procurement = _procurementRepository.GetAll().SingleOrDefault(x=>x.Id==procurementId);

                Product newProduct = new Product() { Company = product.Company, CompanyId = product.CompanyId, ImageUrl = product.ImageUrl, Name = product.Name, Price = product.Price, ProcurementProducts = product.ProcurementProducts, Quantity = product.Quantity };
        ...
        public ProcurementViewModel GetCurrentProcurement(string userId)
        {
            try
            {
                var procurement = _procurementRepository.GetAll().LastOrDefault(x=>x.UserId==userId && x.ProcurementStatus==ProcurementStatus.Процесира);
                if(procurement == null || procurement.ProcurementStatus != ProcurementStatus.Процесира)
                {
        ...
        public int ChangeStatus(int procurementId, string userId, ProcurementStatusViewModel status)
        {
            try
            {
                ...
                if (status == ProcurementStatusViewModel.Одобрено)
                {
                    ...
                    _procurementRepository.Insert(new Procurement()
                    {
                        ...
                        User = user,
                        ProcurementStatus = ProcurementStatus.Процесира
        ...
        public int ChangeQuantity(int procurementId, string userId, int productId, int quantity)
        {
            ...
            var product = procurement.ProcurementProducts.OrderByDescending(p => p.ProductId == productId && p.ProcurementId == procurementId).FirstOrDefault();
        ...
        public int UpdateProcurementInfo(ProcurementViewModel procurementModel)
        {
            ...
            procurement.UserId = procurementModel.User.Id;
            procurement.User = _userRepository.GetById(procurement.UserId);
